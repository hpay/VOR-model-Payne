...
%
% [h, hline, hpoints] = errorb(X,Y,'Parameter','Value',...) see below
%
%% Optional Parameters
...
%    points: will plot the points as well, in the same colors. Specify
%    marker style next
%    line: plot a line connecting data points as well, specify width next
...
function [h, hline, hpoints] = errorb(x,y,varargin)
...
if ~fillFlag % plot errobars normally.
    ...
    for k=1:num2Plot
        ...
        if horizontalFlag
            ...
            if logscale
                ...
                esy = barWidth/2 * 10^(y(k));%*** HP
            end
            
            %       the main line
            if ~topFlag || x(k)>=0  %also plot the bottom half.
                h(end+1) = plot([x(k)+ex x(k)],[y(k) y(k)],'color',lineStyleOrder{k},'linewidth',linewidth,'Clipping',clipping);
                %       the hat
                h(end+1) = plot([x(k)+ex x(k)+ex],[y(k)+esy y(k)-esy],'color',lineStyleOrder{k},'linewidth',linewidth,'Clipping',clipping);
            end
            if ~topFlag || x(k)<0  %also plot the bottom half.
                h(end+1) = plot([x(k) x(k)-ex],[y(k) y(k)],'color',lineStyleOrder{k},'linewidth',linewidth,'Clipping',clipping);
                h(end+1) = plot([x(k)-ex x(k)-ex],[y(k)+esy y(k)-esy],'color',lineStyleOrder{k},'linewidth',linewidth,'Clipping',clipping);
                %rest?
            end
            ...
            if logscale
                ...
                esxR = 10^(barWidth+log10(x(k))) - x(k);
                esxL = x(k) - 10^(log10(x(k))-barWidth);
            ...
            if ~topFlag || y(k)>=0 %also plot the bottom half.
                h(end+1) = plot([x(k) x(k)],[y(k)+ey y(k)],'color',lineStyleOrder{k},'linewidth',linewidth,'Clipping',clipping);
                %       the hat
                if hatFlag
                h(end+1) = plot([x(k)+esxR x(k)-esxL],[y(k)+ey y(k)+ey],'color',lineStyleOrder{k},'linewidth',linewidth,'Clipping',clipping);
                end
            end
            if ~topFlag || y(k)<0 %also plot the bottom half.
                h(end+1) = plot([x(k) x(k)],[y(k) y(k)-ey],'color',lineStyleOrder{k},'linewidth',linewidth,'Clipping',clipping);
                if hatFlag
                h(end+1) = plot([x(k)+esxR x(k)-esxL],[y(k)-ey y(k)-ey],'color',lineStyleOrder{k},'linewidth',linewidth,'Clipping',clipping);
                end
            end
    ...
    %% Plot the line
    if lineFlag
        hline = plot(x,y,'LineWidth',linewidth2,'Color',lineStyleOrder{1},'Clipping',clipping);
    else
        hline = [];
    end
    ...
    if pointsFlag
        for k=1:num2Plot
            if fillmarker
                markercolor = lineStyleOrder{k};
            else
                markercolor = 'w';
            end
            hpoints(end+1) = plot(x(k),y(k),markertype,'markersize',6,'color',lineStyleOrder{k},'MarkerFaceColor',markercolor,'Clipping',clipping);
        end
    end
    ...
    %     Plot the mean:
    plot(x,y,'linewidth',2,'color',color2Plot,'Clipping',clipping);
    %     Make the polygon:
    
    xPoly = [x  fliplr(x) x(1)];
    yPoly = [y+e fliplr(y-e) y(1)+e(1)];
    
    plot(xPoly,yPoly,'color',color2Plot,'Clipping',clipping);
