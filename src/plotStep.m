...
subplot(3,1,1) % Head
plot(tt_step, head_curr,'k','Clipping','off'); hold on
ylabel('Head')
set(gca,'XColor','w');
ylim(-flip(ylims))
plot(max(xlims)+[0 0], 2+[0 10],'k'); % 10 deg/s scale bar
% title(sprintf('T_{smooth} %g s', T_smooth));
axis off

subplot(3,1,2) % Eye
plot(tt_step, Ehat_orig, 'Color', learn_color(2,:),'Clipping','off'); hold on
plot(tt_step, Ehat_learn2, 'Color', learn_color(3,:),'Clipping','off'); hold on
plot(tt_step, Ehat_learn0, 'Color', learn_color(1,:),'Clipping','off'); hold on
ylabel('Eye velocity (deg/s')
set(gca,'XColor','w');
ylim(ylims)
plot(T_step+[-0.1 0], -19+[0 0],'k')

axis off

subplot(3,1,3) % PC
plot(tt_step, Phat_orig,  'Color', learn_color(2,:),'Clipping','off'); hold on
plot(tt_step, Phat_learn2, 'Color', learn_color(3,:),'Clipping','off'); hold on
plot(tt_step, Phat_learn0, 'Color', learn_color(1,:),'Clipping','off'); hold on
ylabel('PC (sp/s)')
ylim(ylims)
...
for jj = 1:3
    subplot(3,1,jj); hold on; h = plot(tt_step, tt_step*0,'k--','LineWidth',.5); uistack(h, 'bottom'); 
end
