...
function [fH_vord, fH_canc, fH_traces, P_gain_vor_mean, P_gain_canc_mean] =...
    plotVorCancSplit(I,  K1, K2, K0, pop, B, S)
...
mplot =  70;
nplots = 4;
fig_pos = [200   100   nplots*pwidth + medge*2+mplot*(nplots-3)  pwidth + medge*2];
ytext1 = {'Purkinje cell response'; 'sp/s per deg/s'};
ytext2 = {'Purkinje cell response'; 'sp/s per deg/s'};
ytext3 = {'PC during VOR cancellation'; '(apparent head sensitivity)'};
xtext3 = {'PC during pursuit'; '(apparent eye sensitivity)'};
...
c_source = cat(3, [0 0 0],  [247 148 30]/255,  I.c(end,:)); % new color scheme
c_source = [lighten(c_source, .7); lighten(c_source, .4); c_source];
if swap_order
    c_source = c_source(:,:,[1 3 2]);
end
...
for jj = 1:3
    ...
    % SWAP EFFERENCE AND VISUAL ORDER FOR PLOTS
    if swap_order
        Phat_source_vord = Phat_source_vord(:,[1 3 2],:,:);
    end
    ...
    [Ehat_canc(:, jj), Phat_canc(:,jj,:), Phat_source_canc] = modelClosedloop(K_pop, I, head_curr, target_curr,...
        light_on,  sine_on);
    if swap_order
        Phat_source_canc = Phat_source_canc(:,[1 3 2],:,:);
    end
    ...
    if split_target_retina
        for ii = 1:size(Phat_source_canc_mean,2) % Loop over head, efference, visual
        ...
        for ii = 1:size(Phat_source_canc_mean2,2) % Loop over head,  efference, visual
    ...
    [Ehat, Phat, Phat_source_purs] = modelClosedloop(K_pop_purs, I, head_curr, target_curr,...
        light_on,  sine_on);
    if swap_order
        Phat_source_purs = Phat_source_purs(:,[1 3 2],:,:);
    end
    for cc = 1:ncells
...
for gg = 1:2 % Loop over gain down and gain up
    ...
    %% EDIT TO PLOT ALL three learning conditions together
    
    % Plot VORD modulation of PC firing
    fH_vord(gg) = figure('Pos',fig_pos);
    left1 = 185;
    
    hs_vor(gg,2) = axes( 'Units','pix', 'Position',[left1, medge, pwidth, pwidth]) ;
    color = I.learn_color;
    plot_conds = [0 1]+gg;
    for jj = plot_conds
        bar(xplot(gg,jj-(gg-1)), P_gain_vor_mean(jj), .8,'EdgeColor','none',...% ,c_edge_curr(jj,:)
            'FaceColor',color(jj,:),'LineWidth',lw); hold on;
    end
    if error_bars
    ...
    %%  Plot contributions of Head, Ret slip, and Eye feedback to PC modulation during VORD
    left2 = 325;
    hs_vor(gg,3) = axes('Units','pix', 'Position',[left2, medge, pwidth*1.6, pwidth]);
    ...
    for ii = 1:3 % which input (row)
        for jj = plot_conds% which learning cond (col)
            plot_curr = P_gain_vor_source(ii,jj,1);
            if plot_curr~=0
                bar(xs(ii,jj-(gg-1)), plot_curr, wb, ...
                    'EdgeColor','none','FaceColor',c_source(jj,:,ii),... % c_edge_curr(jj,:,ii)
                    'Clipping','off','LineWidth',lw); hold on;
            end
        end
    end
    
    xlims = [min(xs(:))-1 max(xs(:))+1];
    ylabel(ytext2,'FontSize',label_font)
    set(gca,'YTick',min(ylims_vord):.4:max(ylims_vord));
    box off; drawnow
    fixticks;
    drawnow;
    learn_str_curr = learnStr(plot_conds(xplot(gg,:)));
%     learn_str_curr = cellfun(@(x) x(1), learn_str_curr,'Uni',0);
    set(gca,'XTick',sort(xs(:)),'XTickLabel',repmat(learn_str_curr, 1,3))
    set(gca,'XLim',xlims,'YLim',ylims_vord)
    
end

if K1.PF==1
    for gg = 1:length(fH_vord)
        axes(hs_vor(gg,3))
        set(gca,'YLim',[-2.3 2.3],'YTickMode','auto');
        %         axes(hs_vor(gg,2))
        %         set(gca,'YLim',[-2.3 2.3],'YTickMode','auto');
        %         cleanticks('y')
    end
...
for gg = 1:2 % Loop over gain down and gain up
    ...
    fH_canc(gg) = figure('Pos',fig_pos);
    left3 = 40;
    hs_canc(gg,1) = axes('Units','pix', 'Position',[left3, medge, pwidth, pwidth]);
    color = I.learn_color;
    ...
    for jj = [0 1]+gg %  low norm high plot order
        ...
        % Plot cells
        msize = 3;
        h(jj) = scatter(P_sens_purs(jj,:), P_sens_canc(jj,:),msize^2, color(jj,:),'LineWidth',.5);
    end
    
    hs = gca;
    hs.XRuler.TickLabelGapOffset = 0;
    hs.YRuler.TickLabelGapOffset = 0;
    
    %     if
    
    ylabel(ytext3,'FontSize',label_font)
    xlim(xlims); ylim(xlims);
    set(gca,'XTick',0:1:xlims(2), 'YTick',0:1:xlims(2))
    axis square;
    fixticks;
    
    xlh =  xlabel(xtext3,'FontSize',label_font);
    drawnow;
    xlh.Position(2) = xlh.Position(2) + 0.1;
    
    
    %% Plot cancellation modulation, or slope of relationship
    xplot = [2 1; 1 2];
    ...
    hs_canc(gg,2) = axes( 'Units','pix','Position',[left1, medge, pwidth, pwidth]);
    color = ones(3,3);
    c_curr = I.learn_color;
    
    plot_conds = [0 1]+gg;
    for jj = plot_conds%  low norm high plot order
        bar(xplot(gg,jj-(gg-1)), mean(P_plot(jj,:)),.8,'EdgeColor','none',...
            'FaceColor',c_curr(jj,:),'LineWidth',lw); hold on;
    end
    if error_bars
    ...
    %%  Plot contributions of Head, Ret slip, and Eye feedback to PC modulation during cancellation - PLOT TARGET AND RS CONTRIBUTIONS
    hs_canc(gg,3) = axes( 'Units','pix','Position',[left2, medge, pwidth*1.6, pwidth]);
    ...
    if swap_order
        labels_source = {'Head','Efference','Visual'};
    else
    ...
    for ii = 1:3 % Loop of head, visual, efference
        for jj = plot_conds % Loop of Low Normal High gain
            ...
            %                 'EdgeColor',c_edge_curr(jj,:,ii),'FaceColor','none',...
            %                 'Clipping','off','LineWidth',2); hold on
            %
            plot_curr = nansum(yy);
            if plot_curr~=0
                h = bar(xs(ii,jj-(gg-1)), plot_curr,wb,'Stacked',...
                    'EdgeColor','none','FaceColor',c_curr(jj,:,ii),...
                    'Clipping','off','LineWidth',lw); hold on
            end
        ...
        % Plot labels
        text(mean(xs(ii,:)), ylims_canc(2)*.9,labels_source{ii},'Hor','center');
        hs  = gca;
        hs.XRuler.TickLabelGapOffset = 0;
...
for jj =1:3 % Loop of Low Normal High gain
    ...
    plot(tt, -2+scale_small(1)*Phat_source_canc_mean(:,strcmp(labels_source,'Visual'),1,jj),'Color',colors(jj,:)); hold on;
    
    text(xt,-4, sprintf('Target x %g', scale_small(2)),'Color','r')
    plot(tt, -4+scale_small(2)*Phat_source_canc_mean(:,strcmp(labels_source,'Visual'),2,jj),'Color',colors(jj,:)); hold on;
    
    text(xt,-6.32, sprintf('Efference copy x %g',scale_small(3)),'Color','r')
    plot(tt, -6+scale_small(3)*Phat_source_canc_mean(:,strcmp(labels_source,'Efference'),1,jj),'Color',colors(jj,:)); hold on;
    
    
    text(xt,-10, 'Purkinje total','Color','r')
    Phat_total = sum(sum(Phat_source_canc_mean(:,1,1,jj),2),3);
